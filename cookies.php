 ...
 */
function impronta_cookies_fail(int $status, string $message): void {
...
if ($path === '' || strpos($path, '..') !== false || substr_count($path, '/') !== 1) {
    impronta_cookies_fail(400, 'ruta no valida');
}

$denied = token::authorize($path, $token, (int) $expires, (int) $courseid, (int) $userid);
if ($denied !== null) {
    impronta_cookies_fail(403, get_string($denied, 'filter_impronta'));
}

$reason = null;
$signature = reelo_api::signature($path, $reason, (int) $userid);
if (!$signature) {
    impronta_cookies_fail(502, 'sin firma: ' . ($reason ?: 'desconocido'));
}
...
if (!cloudfront::set_cookies($signature)) {
    ...
    impronta_cookies_fail(409, 'sin dominio comun; usar urls firmadas');
}
